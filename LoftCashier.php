                        ...
                        <div class="col-lg-8 col-md-7">
                            ...
                            <div class="header">
                              <h4 class="title" style="font-family: 'Trirong', serif;">แผนผังร้าน ลอฟท์</h4>
                                ...
                                <br><p id="rcorners2">โซนแคชเชียร์</p>
                                        ...
                                        <table width="100%" border="2" style="border: solid #DCDCDC;">
                                          <tr>
                                            ...
                                            <tr>
                                                ...
                                                $sql = "SELECT * FROM (SELECT tablemantraplan.*,bookingres.BKStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE bookingres.DataIn = '$_SESSION[SelectDate]' AND bookingres.TimeRange = '$_SESSION[SelectTime]' AND tablemantraplan.TableZone ='8' AND tablemantraplan.TableGroup ='A' AND tablemantraplan.ResName ='Loft' UNION All SELECT tablemantraplan.*,tablemantraplan.TableStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE TableZone ='8' AND TableGroup ='A' AND tablemantraplan.ResName ='Loft') As TBid GROUP BY TBid.TableID ORDER BY TBid.Orders ASC";
                                            ...
                                            <tr>
                                                ...
                                                $rn = 0;
                                                $PathIcon = "images/MantraPlan/TableIcon/";
                                                $sql = "SELECT * FROM (SELECT tablemantraplan.*,bookingres.BKStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE bookingres.DataIn = '$_SESSION[SelectDate]' AND bookingres.TimeRange = '$_SESSION[SelectTime]' AND tablemantraplan.TableZone ='8' AND tablemantraplan.TableGroup ='B' AND tablemantraplan.ResName ='Loft' UNION All SELECT tablemantraplan.*,tablemantraplan.TableStatus FROM tablemantraplan LEFT JOIN bookingres ON tablemantraplan.TableID = bookingres.TableID WHERE TableZone ='8' AND TableGroup ='B' AND tablemantraplan.ResName ='Loft') As TBid GROUP BY TBid.TableID ORDER BY TBid.Orders ASC";
                                                $query = mysqli_query($conn,$sql);
                                                while($result=mysqli_fetch_array($query,MYSQLI_ASSOC))
	                                               {
                                                    $NewVar = (int)substr($result["TableNumber"], 3, 2);
